...
export const createNotification = async ({ userId, questionId, questionText, totalVotes, type, message }) => {
  if (!userId) return;

  // Direct notification (e.g. comment)
  if (type && message) {
...
// ── NotifIcon ─────────────────────────────────────────────────
const typeIcon = (type) => type === 'milestone' ? '🎉' : type === 'comment' ? '💬' : '🗳️';
