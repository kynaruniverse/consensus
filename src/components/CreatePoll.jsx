    ...
    <form onSubmit={handleSubmit} className="card space-y-4">
      ...
      <div className="grid grid-cols-1 gap-2">
        {options.map((opt, i) => (
          <div key={i} className="flex gap-2">
            ...
            />
            {options.length > 2 && (
              <button type="button" onClick={() => removeOption(i)} className="p-2 text-slate-500 hover:text-red-400">
                <X size={18} />
              </button>
            )}
          </div>
      ...
      <div className="flex justify-between items-center pt-2">
        <button type="button" onClick={addOption} disabled={options.length >= 4}
          className="text-xs font-black uppercase tracking-widest text-sky-400 disabled:opacity-30 flex items-center gap-1">
