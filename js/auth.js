...
// ProfilePage — view & edit profile, see stats, sign out
...
export const AuthPage = ({ onAuth }) => {
  ...
  const signIn = async () => {
    ...
    onAuth({ ...data.user, username: profile?.username, age_range: profile?.age_range, gender: profile?.gender });
  };

  const signUp = async () => {
    if (!email || !password || !username) { setError('Email, password and username are required.'); return; }
    ...
    if (data.session) {
      onAuth({ ...data.user, username, age_range: ageRange||null, gender: gender||null });
    } else {
  ...
  return div({ className:'page fade-up', style:{maxWidth:480} },
    ...
    div({ className:'tab-bar' },
      e('button', { className:'tab-btn'+(tab==='signin'?' active':''), onClick:()=>{setTab('signin');setError('');} }, 'Sign In'),
      e('button', { className:'tab-btn'+(tab==='signup'?' active':''), onClick:()=>{setTab('signup');setError('');} }, 'Sign Up')
    ),
    ...
    div({ className:'card', style:{padding:24} },
      ...
      tab==='signup' && e('input', { ...inp, type:'text', placeholder:'Username (public)', value:username,
        onChange:ev=>setUsername(ev.target.value.replace(/\s/g,'').toLowerCase()) }),

      e('input', { ...inp, type:'email', placeholder:'Email address', value:email,
        onChange:ev=>setEmail(ev.target.value) }),
      e('input', { ...inp, type:'password', placeholder:'Password (min 6 chars)', value:password,
        onChange:ev=>setPassword(ev.target.value) }),

      // Optional demographics on sign up
      tab==='signup' && div(null,
...
export const ProfilePage = ({ user, onSignOut, onProfileUpdate }) => {
  ...
  const save = async () => {
    setSaving(true);
    const { error } = await db.from('profiles').update({ age_range:ageRange||null, gender:gender||null }).eq('id', user.id);
    setSaving(false);
  ...
  return div({ className:'page fade-up' },
    ...
    div({ style:{display:'flex',alignItems:'center',gap:16,marginBottom:28} },
      ...
      div(null,
        ...
        user.age_range && p({ style:{color:'#475569',fontSize:13,marginTop:2} },
          user.age_range + (user.gender ? ' · ' + user.gender : '')
        )
      )
    ),
    ...
    e('button', { className:'btn-danger', onClick:signOut }, '→ Sign out'),
