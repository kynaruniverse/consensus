    ...
    div({style:{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8,marginBottom:12}},
      ...
      div({style:{display:'flex',gap:4,flexShrink:0,marginTop:2}},
        trending && span({style:{fontSize:10,fontWeight:700,color:'#f97316',
          ...
          borderRadius:999,padding:'2px 7px'}},'🔥 Trending'),
        hot && !trending && span({style:{fontSize:10,fontWeight:700,color:'#fbbf24',
          background:'rgba(251,191,36,0.12)',border:'1px solid rgba(251,191,36,0.3)',
          borderRadius:999,padding:'2px 7px'}},'HOT')
      )
    ),

    // Bottom row: category + option pills + vote count
    div({style:{display:'flex',justifyContent:'space-between',alignItems:'center',gap:10}},
      div({style:{display:'flex',gap:5,flexWrap:'wrap',flex:1,minWidth:0,alignItems:'center'}},
        cat && cat.id!=='General' && span({style:{fontSize:11,padding:'2px 8px',borderRadius:999,
          border:'1px solid '+cat.color+'40',color:cat.color,background:cat.color+'15',
          fontWeight:600,flexShrink:0}}, cat.label),
...
const Pill = ({label, active, color, onClick}) =>
  e('button',{onClick, style:{
    padding:'6px 14px',borderRadius:999,border:'1px solid',fontSize:12,
  ...
  return div({style:{maxWidth:640,margin:'0 auto',padding:'58px 16px 80px',position:'relative'}},
    ...
    loading
      ...
      ? div({style:{textAlign:'center',padding:'60px 0'},className:'fade-up'},
          ...
          p({style:{color:'#94a3b8',fontSize:18,fontWeight:600,marginBottom:8}},
            activeCat==='All' ? 'No questions yet' : 'No '+activeCat+' questions yet'),
          p({style:{color:'#475569',fontSize:15}},
            activeCat==='All' ? 'Be the first to ask the world something.' : 'Post the first one!')
        )
